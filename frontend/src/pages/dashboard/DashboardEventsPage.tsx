  ...
  return (
    <section className="rounded-xl border border-slate-200 bg-white p-5 sm:p-6">
      ...
      <div className="mt-6 space-y-8">
        {isLoading ? (
          <p className="text-center text-sm text-slate-500">جار تحميل الفعاليات...</p>
        ) : hasError ? (
          <p className="text-center text-sm text-red-600">تعذر تحميل الفعاليات.</p>
        ) : (
          <>
            ...
            {upcomingEvents.length === 0 && finishedEvents.length === 0 ? (
              <p className="text-center text-sm text-slate-500">لا توجد فعاليات حالياً.</p>
            ) : null}
          </>
        )}
      </div>
